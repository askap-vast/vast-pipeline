...
def extract_from_image(
    ...
    **kwargs,
    ...
    df["flux_int"] = flux * 1.0e3
    df["flux_int_err"] = flux_err * 1.0e3
...
def parallel_extraction(
    ...
    intermediate_df = list(
        map(lambda x: {**(mapping.loc[x["image"], :].to_dict()), **x}, forced_dfs)
    )
    ...
    df_out = pd.concat(intermediate_df, axis=0, sort=False).rename(
...
def parallel_write_parquet(
    df: pd.DataFrame, run_path: str, add_mode: bool = False
) -> None:
    """
