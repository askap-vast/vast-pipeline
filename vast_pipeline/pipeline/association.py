...
def calc_de_ruiter(df: pd.DataFrame) -> np.ndarray:
    ...
    ra_1[ra_1 > 270.0] -= 180.0
    ra_2[ra_2 > 270.0] -= 180.0
    ra_1[ra_1 < 90.0] += 180.0
    ra_2[ra_2 < 90.0] += 180.0
...
def one_to_many_basic(
    ...
    duplicated_skyc2 = skyc2_srcs.loc[
        (skyc2_srcs["source"].notnull()) & skyc2_srcs["source"].duplicated(keep=False),
        ["source", "related", "d2d"],
    ]
    ...
    logger.info("Detected #%i double matches, cleaning...", duplicated_skyc2.shape[0])
    ...
    # Get those that need to be given a new ID (i.e. not the min dist_col)
    idx_to_change = duplicated_skyc2.index.to_numpy()[
    ...
    new_source_ids = [generate_shortuuid(UUID_LEN_SOURCE) for _ in range(idx_to_change.shape[0])]
    ...
    not_original = duplicated_skyc2.loc[idx_to_change].copy()

    original = duplicated_skyc2.drop_duplicates("source").copy()
    ...
    original["related"] = original[["related", "source"]].apply(
        add_new_one_to_many_relations, args=(False, new_original_related), axis=1
    )

    not_original["related"] = not_original.apply(
        add_new_one_to_many_relations, args=(False,), axis=1
    )
    ...
    skyc2_srcs.loc[duplicated_skyc2.index.values, "related"] = duplicated_skyc2.loc[
        duplicated_skyc2.index.values, "related"
    ].values
    ...
    duplicated_skyc2 = duplicated_skyc2.loc[duplicated_skyc2.duplicated("source")]
    ...
    duplicated_skyc2 = (
        ...
        .merge(source_df_index_to_copy, left_on="source", right_index=True, how="left")
...
def one_to_many_advanced(
    ...
    logger.debug(
        "Detected #%i one-to-many assocations, cleaning...", duplicated_skyc1.shape[0]
    )
    ...
    original = duplicated_skyc1.drop_duplicates("source_skyc1").copy()
    ...
    # for the original ones.
    # The not original only require the appending of the original index.
    original["related_skyc1"] = original[["related_skyc1", "source_skyc1"]].apply(
        add_new_one_to_many_relations, args=(True, new_original_related), axis=1
    )
    ...
    not_original.loc[:, "related_skyc1"] = not_original.apply(
        add_new_one_to_many_relations, args=(True,), axis=1
    )
    ...
    temp_srcs.loc[
        duplicated_skyc1.index.values, "related_skyc1"
    ] = duplicated_skyc1.loc[duplicated_skyc1.index.values, "related_skyc1"].values

    # Finally we need to create copies of the previous sources in the
    # sources_df to complete the new sources.

    # To do this we get only the non-original sources
    duplicated_skyc1 = duplicated_skyc1.loc[duplicated_skyc1.duplicated("source_skyc1")]
...
def many_to_many_advanced(temp_srcs: pd.DataFrame, method: str) -> pd.DataFrame:
    ...
    logger.debug("Detected #%i many-to-many assocations, cleaning...", m_to_m.shape[0])

    dist_col = "d2d" if method == "advanced" else "dr"
    min_col = "min_" + dist_col

    # get the minimum de ruiter value for each extracted source
    m_to_m[min_col] = m_to_m.groupby("index_old_skyc2")[dist_col].transform("min")
...
def many_to_one_advanced(temp_srcs: pd.DataFrame) -> pd.DataFrame:
    ...
    duplicated_skyc2 = temp_srcs.loc[
        temp_srcs["index_old_skyc2"].duplicated(keep=False), cols
    ]
    ...
    logger.debug("Detected #%i many-to-one associations", duplicated_skyc2.shape[0])
    ...
    duplicated_skyc2 = duplicated_skyc2.merge(
        new_relations, left_on="index_old_skyc2", right_index=True, how="left"
    )
    ...
    duplicated_skyc2["new_relations"] = duplicated_skyc2.apply(
        lambda x: list(set(x["new_relations"]) - set([x["source_skyc1"]])), axis=1
    )

    # Use the 'add_new_many_to_one_relations' method to add tthe new relatitons
    # to the actual `related_skyc1' column.
    duplicated_skyc2["related_skyc1"] = duplicated_skyc2.apply(
        add_new_many_to_one_relations, axis=1
    )
    ...
    ] = duplicated_skyc2.loc[duplicated_skyc2.index.values, "related_skyc1"].values
...
def basic_association(
    ...
    idx, d2d, _ = skyc2.match_to_catalog_sky(skyc1)
    ...
    skyc2_srcs.loc[nan_sel, "source"] = [
        generate_shortuuid(UUID_LEN_SOURCE) for _ in range(nan_sel.sum())
    ]
    ...
    skyc1_srcs = pd.concat(
        [skyc1_srcs, skyc2_srcs[~skyc2_srcs["source"].isin(skyc1_srcs["source"])]],
        ignore_index=True,
...
def advanced_association(
    ...
    temp_skyc1_srcs = (
        skyc1_srcs.loc[idx_skyc1].reset_index().rename(columns={"index": "index_old"})
    )
    temp_skyc2_srcs = (
        skyc2_srcs.loc[idx_skyc2].reset_index().rename(columns={"index": "index_old"})
    )
    ...
    temp_srcs = temp_srcs.drop(["d2d_skyc1", "dr_skyc1", "dr_skyc2"], axis=1).rename(
        columns={"d2d_skyc2": "d2d"}
    )
    ...
    new_sources = skyc2_srcs.loc[
        skyc2_srcs.index.difference(temp_srcs["index_old_skyc2"].values)
    ].reset_index(drop=True)
    # update the src numbers for those sources in skyc2 with no match
    # using the max current src as the start and incrementing by one
    new_sources["source"] = new_sources.apply(lambda _: generate_shortuuid(UUID_LEN_SOURCE), axis=1)
    ...
    sources_df = pd.concat(
        [sources_df, skyc2_srcs_toappend], ignore_index=True
    ).reset_index(drop=True)

    # update skyc1 and df for next association iteration
    # calculate average angles for skyc1
    skyc1_srcs = pd.concat([skyc1_srcs, new_sources], ignore_index=True).reset_index(
        drop=True
    )
...
def association(
    ...
    if parallel:
        ...
        # Deserialise image_dj back to a django model object.
    ...
    if add_mode:
        ...
        if images_df.empty:
            logger.info("No new images found, stopping association%s.", skyreg_tag)
            ...
            return sources_df.drop(["ra", "dec"], axis=1).rename(
                columns={"ra_source": "ra", "dec_source": "dec"}
            )
        logger.info(f"Found {images_df.shape[0]} images to add to the run{skyreg_tag}.")
        ...
        first_images = images_df.loc[
            images_df["epoch"] == unique_epochs[0], "image_dj"
        ].to_list()
    ...
    if unique_epochs.shape[0] == 1 and not add_mode:
        ...
        logger.warning("No images to associate with!%s.", skyreg_tag)
        logger.info("Returning base sources only%s.", skyreg_tag)
    ...
    for it, epoch in enumerate(unique_epochs[start_epoch:]):
        ...
        sources_df.loc[ra_wrap_mask, "ra_wrap"] = (
            sources_df[ra_wrap_mask].ra.values + 360.0
        )
        ...
        tmp_srcs_df = sources_df.loc[
            (sources_df["source"].notnull()) & (sources_df["forced"] == False),
            [
                "ra",
                "dec",
                "uncertainty_ew",
        ...
        wm_uncertainty_ew = 1.0 / np.sqrt(tmp_srcs_df["weight_ew"].sum())

        wm_dec = tmp_srcs_df["interim_ns"].sum() / tmp_srcs_df["weight_ns"].sum()
        wm_uncertainty_ns = 1.0 / np.sqrt(tmp_srcs_df["weight_ns"].sum())
        ...
        ra_wrap_mask = weighted_df.ra >= 360.0
        weighted_df.loc[ra_wrap_mask, "ra"] = (
            weighted_df[ra_wrap_mask].ra.values - 360.0
        )
        ...
        logger.info("Association iteration #%i complete%s.", it + 1, skyreg_tag)
...
def parallel_association(
    ...
    """
    logger.info(
        "Running parallel association for %i sky region groups.", n_skyregion_groups
    )
