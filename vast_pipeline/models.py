...
from vast_pipeline.utils.utils import (
    UUID_ALPHABET, UUID_LEN_SHORT,
    UUID_LEN_SOURCE, UUID_LEN_MEAS
)
...
class Comment(models.Model):
    ...
    id = ShortUUIDField(primary_key=True, editable=False, length=UUID_LEN_SHORT, alphabet=UUID_ALPHABET)
    author = models.ForeignKey(User, on_delete=models.CASCADE)
    datetime = models.DateTimeField(auto_now_add=True)
    comment = models.TextField()
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE)
    object_id = ShortUUIDField(editable=False, length=UUID_LEN_SHORT,
                               max_length=UUID_LEN_MEAS, alphabet=UUID_ALPHABET)
...
class Run(CommentableModel):
    ...
    description = models.CharField(
        max_length=240, blank=True, help_text="A short description of the pipeline run."
    )
    time = models.DateTimeField(auto_now=True, help_text="Datetime of a pipeline run.")
    path = models.FilePathField(max_length=200, help_text="path to the pipeline run")
    STATUS_CHOICES = [
        ("INI", "Initialised"),
        ("QUE", "Queued"),
        ("RUN", "Running"),
    ...
    n_images = models.IntegerField(
        default=0, help_text="number of images processed in this run"
    )
    n_sources = models.IntegerField(
        default=0, help_text="number of sources extracted in this run"
    )
    n_selavy_measurements = models.IntegerField(
        default=0, help_text="number of selavy measurements in this run"
    )
    n_forced_measurements = models.IntegerField(
        default=0, help_text="number of forced measurements in this run"
    )
    n_new_sources = models.IntegerField(
        default=0, help_text="number of new sources in this run"
    )
    ...
    objects = (
        RunManager()
    )  # used instead of RunQuerySet.as_manager() so mypy checks work
...
class SourceQuerySet(models.QuerySet):
    def cone_search(self, ra: float, dec: float, radius_deg: float) -> models.QuerySet:
        ...
        """
        return self.extra(
            select={"distance": "q3c_dist(wavg_ra, wavg_dec, %s, %s) * 3600"},
            select_params=[ra, dec],
...
class Image(CommentableModel):
    ...
    measurements_path = models.FilePathField(
        max_length=200,
        db_column="meas_path",
        help_text=("the path to the measurements parquet that belongs to this image"),
    )
    ...
    name = models.CharField(unique=True, max_length=200, help_text="Name of the image.")
    path = models.FilePathField(
        max_length=500, help_text="Path to the file containing the image."
    )
...
class Measurement(CommentableModel):
    ...
    source = models.ManyToManyField(
        "Source", through="Association", through_fields=("meas", "source")
    )
    ...
    flux_peak_isl_ratio = models.FloatField(
        help_text=("Ratio of the component peak flux to the total" " island peak flux.")
    )
    chi_squared_fit = models.FloatField(
        db_column="chi2_fit", help_text="Chi-squared of the Guassian fit to the source."
    )
    spectral_index = models.FloatField(
        db_column="spectr_idx", help_text="In-band Selavy spectral index."
    )
    spectral_index_from_TT = models.BooleanField(
        default=False,
        db_column="spectr_idx_tt",
        help_text=("True/False if the spectral index came from the taylor " "term."),
    )

    local_rms = models.FloatField(help_text="Local rms in mJy from Selavy.")  # mJy/beam
    ...
    forced = models.BooleanField(
        default=False, help_text="True: the measurement is forced extracted."
    )
...
class Source(CommentableModel):
    ...
    id = ShortUUIDField(primary_key=True, editable=False, length=UUID_LEN_SOURCE, alphabet=UUID_ALPHABET)
    ...
    def get_measurement_pairs(self) -> List[MeasurementPair]:
        ...
        for meas_a, meas_b in combinations(measurements, 2):
            ...
            vs_peak = calculate_vs_metric(
                meas_a.flux_peak,
                meas_b.flux_peak,
                meas_a.flux_peak_err,
                meas_b.flux_peak_err,
            )
            ...
            measurement_pairs.append(
                MeasurementPair(
                    self.id, meas_a.id, meas_b.id, vs_peak, m_peak, vs_int, m_int
                )
            )
...
class Association(models.Model):
    ...
    d2d = models.FloatField(
        default=0.0, help_text="astronomical distance calculated by Astropy, arcsec."
    )
