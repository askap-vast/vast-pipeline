...
def delete_pipeline_run_raw_sql(p_run):
    ...
    with connection.cursor() as cursor:
        ...
        sql_cmd = "SELECT skyregion_id FROM vast_pipeline_skyregion_run WHERE run_id = {p_run_id};"
        _run_raw_sql(sql_cmd, cursor, debug=True)
        ...
        for sky_id_tuple in sky_ids:
            ...
            sql_cmd = "DELETE FROM vast_pipeline_skyregion_run WHERE skyregion_id = {sky_id} AND run_id = {p_run_id};"
            _run_raw_sql(sql_cmd, cursor, debug=True)
            try:
                sql_cmd = "DELETE FROM vast_pipeline_skyregion WHERE id = {sky_id};"
                _run_raw_sql(sql_cmd, cursor, debug=True)
            except Exception as e:
                print("++++++++++++++++++++++++++++++++")
                print(e, sky_id)
        ...
        sql_cmd = "DELETE FROM vast_pipeline_run WHERE id = {p_run_id};"
        _run_raw_sql(sql_cmd, cursor)
