...
def association(p_run, images, meas_dj_obj, limit, max_limit, bw_limit, method):
    if method == 'basic':
        sources_df = association_simple(
            p_run,
            images,
            meas_dj_obj,
            limit
...
def association_simple(p_run, images, meas_dj_obj, limit):
...
def association_advanced(p_run, images, meas_dj_obj, limit, max_limit, bw_limit):
    # read the needed sources fields
    cols = [
        'id',
        'ra',
    ...
    for it, image in enumerate(images[1:]):
        ...
        # match the new sources to the base
        # idx gives the index of the closest match in the base for skyc2
        idx_skyc1, idx_skyc2, d2d, d3d = skyc2.catalog.search_around_sky(skyc1, max_limit)
        # Apply the beamwidth limit 
        sel = d2d <= bw_max

        # make a temp dataframe to do the analysis
        ...
        temp_srcs = temp_skyc1_srcs.merge(
            temp_skyc2_srcs, left_index=True, right_index=True
        )
        temp_srcs['d2d'] = d2d[sel]
...
def calculate_de_ruiter(row):
    ...
    ra_1_err = row.ra_err_x
    ra_2_err = row.ra_err_y
    
    dec_1 = row.dec_x
    dec_2 = row.dec_y

    dec_1_err = row.dec_err_x
    dec_2_err = row.dec_err_x
