...
The installation instructions are the same as the ones describes in the [quickstart section](../quickstart/installation.md).
...
This guide explain the installation, compilation and development of the front end assets (HTML, CSS, JS and relative modules). We make use of a `node` installation with `npm` and `gulp` tasks.
